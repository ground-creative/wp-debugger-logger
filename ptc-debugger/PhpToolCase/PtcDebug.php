		...
		*/
		protected static function _buildVarsPanel()
		{
			...
			if(!@empty($included_files))
			{
				...
				{ 
					$div.=$a.' '.@self::_buildTraceLink($filename).$filename.'</a>';
		...
		*/
		protected static function _includeCss()
		{
			return self::_compressHtml(
				'<style type="text/css">
					#topRight{font-family:Arial,sant-serif;position:fixed;top:'.self::$_options['panel_top'].
					';right:'.self::$_options['panel_right'].';background:#eee;color:#333;z-index:10000;}
				...
				'<script>
					...
					function read_code(filename,line) 
					{
						var query="http://'.addslashes($_SERVER['HTTP_HOST']).
							$path=addslashes(str_replace($_SERVER['DOCUMENT_ROOT'],'',
